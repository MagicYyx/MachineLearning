...
class RandomForest(object):
    ...
    def __model_evaluation(self):
        if self.__type == 'Classifier':
            print('随机森林训练集的预测准确率:', metrics.accuracy_score(self.__y_train, self.__random_forest.predict(self.__x_train)))
            print('随机森林测试集的预测准确率:', metrics.accuracy_score(self.__y_test, self.__random_forest.predict(self.__x_test)))
            # 绘制ROC曲线
